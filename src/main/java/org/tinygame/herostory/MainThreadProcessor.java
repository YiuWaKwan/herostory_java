...
public final class MainThreadProcessor {
     ...
     * @param msg 消息对象
     */
    public void process(ChannelHandlerContext ctx, GeneratedMessage msg) {
        if (null == ctx ||
            null == msg) {
            return;
        }
        ...
        _es.submit(() -> {
            // 获取指令处理器
            ICmdHandler<? extends GeneratedMessage>
     ...
     * @return 指令对象
     */
    static private <TCmd extends GeneratedMessage> TCmd cast(Object msg) {
        if (null == msg ||
            !(msg instanceof GeneratedMessage)) {
            return null;
        } else {
            return (TCmd) msg;
        }
